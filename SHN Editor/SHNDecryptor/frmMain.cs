...
namespace SHNDecrypt
{
    ...
    {
        Search.searchFindOld searchFrm;
        Search.searchFilter filterFrm;
        ...
        void InitFile(string Filename, bool NewFile = false)
        {
            ...
            if (NewFile == false) 
            {
                if (recentToolStripMenuItem.DropDownItems.Count == 7)
                {
                    ...
                    mruManager.AddRecentFile(Filename);
                }
                else
                {
                    mruManager.AddRecentFile(Filename);
        ...
        public SHNFile file
        {
            get
            {
                try { return (SHNFile)dataGrid.Tag; }
                catch { /*MessageBox.Show("No file found");*/ return null; }
            }
            set
            {
                try { dataGrid.Tag = value; }
                catch { }
            }
        }
        ...
        public DataGridView dataGrid
        {
            get
            {
                try { return (DataGridView)FileTabs.SelectedTab.Tag; }
                catch { return null; }
            }
            set
            {
        ...
        private void searchToolStripMenuItem_Click(object sender, EventArgs e)
        {
           ...
           searchFrm.Text = "Find: " + FileTabs.SelectedTab.Text;
        ...
        private void findOldToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (FileTabs.TabPages.Count < 1) return;
            if (searchFrm == null)
                searchFrm = new Search.searchFindOld(this);
            searchFrm.Text = "Find: " + FileTabs.SelectedTab.Text;
            //searchFrm.Init();
            searchFrm.Show();
        }

        private void resetColumnSortToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void filterToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (FileTabs.TabPages.Count < 1) return;
            if (filterFrm == null)
                filterFrm = new Search.searchFilter(this);
            filterFrm.Text = "Filter: " + FileTabs.SelectedTab.Text;
            filterFrm.Show();
        }
    }
}
