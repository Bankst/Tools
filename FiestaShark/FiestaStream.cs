...
namespace FiestaShark {
	public sealed class FiestaStream {
		...
		public FiestaPacket Read(DateTime pTransmitted) {
			if (mCursor < 1) return null;
			ushort packetLen = 0;
			byte HeaderLen = 1;
			if ((packetLen = mBuffer[0]) == 0) //big packet
			{
				packetLen = BitConverter.ToUInt16(mBuffer, 1);
				HeaderLen = 3;
			}
			if (mCursor < (packetLen + HeaderLen)) return null;	//not enough data
			byte[] packetData = new byte[packetLen];
			Buffer.BlockCopy(mBuffer, HeaderLen, packetData, 0, packetLen);
			if (encrypt) Decrypt(packetData);
		...
		void Decrypt(byte[] data) {
			for (int i = 0; i < data.Length; i++) {
				data[i] ^= FiestaCrypto.xorTable[XorK];
				XorK++;
				if (XorK == 499) XorK = 0;
			}
		}
	...
	public class FiestaCrypto {
		public static byte[] xorTable = new byte[499]
		{
